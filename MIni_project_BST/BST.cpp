...
public:
    ...
    void draw_node(int item, int centre_x, int centre_y, bool write_value = true) {
        char data_string[10];
        glBegin(GL_POLYGON);
        glVertex2f(static_cast<GLfloat>(centre_x + node_width), static_cast<GLfloat>(centre_y + node_width));
        glVertex2f(static_cast<GLfloat>(centre_x - node_width), static_cast<GLfloat>(centre_y + node_width));
        glVertex2f(static_cast<GLfloat>(centre_x - node_width), static_cast<GLfloat>(centre_y - node_width));
        glVertex2f(static_cast<GLfloat>(centre_x + node_width), static_cast<GLfloat>(centre_y - node_width));
        glEnd();
    ...
    NODE remove(int item) {
        ...
        int num_of_nodes = 1;
        int node_x = root_centre_x, node_y = root_centre_y;
        //int par_x = par_x,par_y = par_y;
        NODE temp = root, par = root;//new_node = new_node;
        if (root == NULL) {
            glColor3f(1.0, 0.0, 0.0);
            display_string(not_found, 100, 600);
            return root;
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (digit == 0 && key == '-') {
        ...
        s[1] = '\0';
        glColor3f(1.0, 0.0, 0.0);
        display_string(s, static_cast < float>(420 + (15 * digit)), 600);
        glFlush();
        sign = -1;
    }
